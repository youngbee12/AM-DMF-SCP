...
hela_hand_sample<-sc_group %>% arrange(Group)%>%filter(Group=="outchip"|Group=="Hela"&batch=="batch1") 
data_hela_hand <-sc_data[,hela_hand_sample$sample]
...
data_30<-filter_na_bygroup(data_hela_hand,group_info = dplyr::select(hela_hand_sample,c("sample","Group")),x=0.5)
#Knn补缺
library(impute)
...
data_hela_hand<-half_data_meadian_impute
...
hela_hand_sample %>% filter(Group=="hand") %>%.[,"p_num"]
hela_hand_sample %>% filter(Group=="Hela") %>%.[,"p_num"]

wide_df_hela_hand<-wide_df %>% filter(Group=="outchip"|Group=="Hela") %>%.[,c(hela_hand_sample$sample,"Group","mean","sd")]
wide_df_hela_hand[wide_df_hela_hand=="Hela"] <- "inchip"
filter_data_hela_hand<-hela_hand_sample
library(ggpubr)
RES<-compare_means(p_num~Group,filter_data_hela_hand,ref.group = "outchip", method = "t.test")
values=wheel("steelblue", num = 4)[1:2]
pdf(file="./analysis/hela_hand/20231009_identi_hela_hand.pdf",height = 5,width = 5 )


ggplot(wide_df_hela_hand, aes(x=Group, y=mean,color=Group,fill=Group)) +
  ...
  geom_jitter(data = filter_data_hela_hand,aes(x=Group,y=p_num),width=.1)+
  scale_y_continuous(limits=c(0,3100),breaks=seq(0,3000,750))+
  geom_text(aes(label = ceiling(mean)), vjust = -9, colour = "black")+
  #stat_compare_means(aes(label=..p.signif..), label.x = 1.5, label.y = 40)
 stat_compare_means(data =filter_data_hela_hand,aes(x=Group, y=p_num),method = "t.test",na.rm = T)+      # Add global p-value
  #stat_compare_means(data =filter_data_hela_hand ,aes(x=Group, y=p_num),label = "p.signif", method = "t.test", ref.group = "outchip")+     
  ylab("Proteins")+
  xlab("")+
  theme_bw() +
...
pdf(file = "./analysis/hela_hand/20231011_corr_hela_hand.pdf",height = 5,width = 6)


corrplot(cor(data_hela_hand,method="pearson",use="complete.obs"),
         method="color",tl.col = "black",
         
         col=c(rep("white", 500),rep('grey', 300) ,COL2('RdYlBu', 200)),
         col.lim=c(0.8,1),
         #addCoef.col = "white",
         #type="upper")
)
...
#cv
library(rknn)
cv_outchip_pro<-apply(data_hela_hand[listn[[2]],7:12],1,function(x)cv.coef(as.numeric(x)))
cv_inchip_pro<-apply(data_hela_hand[listn[[1]],1:6],1,function(x)cv.coef(as.numeric(x)))

median(cv_outchip_pro,na.rm = T)
median(cv_inchip_pro,na.rm = T)
length(cv_outchip_pro[cv_outchip_pro<2])

length(cv_inchip_pro[cv_inchip_pro<2])

data_outchip_pro<-data_hela_hand[listn[[2]],7:12]
data_outchip_pro$cv<-cv_outchip_pro
data_outchip_pro$mean<-apply(data_outchip_pro[,1:6],1,function(x)mean(x,na.rm=T))


data_inchip_pro<-data_hela_hand[listn[[1]],1:6]
data_inchip_pro$cv<-cv_inchip_pro
data_inchip_pro$mean<-apply(data_inchip_pro[,1:6],1,function(x)mean(x,na.rm=T))


ggplot(data_inchip_pro, aes(x = log2(mean), y = cv))+
  geom_point(size=0.5) +
  geom_pointdensity() +
  # scale_color_viridis(discrete = F)+
  #scale_color_distiller(palette= 3, direction=1) +
  #geom_smooth(method = "lm", se = FALSE, col = "red", linetype = "dashed",linewidth=0.5) +
# geom_text(aes(x = 10, y = 20, label = paste0("r = ", round(r, 2), ", p < 0.0001 ")),
...
ggplot(data_outchip_pro, aes(x = log2(mean), y = cv))+
  geom_point(size=0.5) +
  geom_pointdensity() +
  # scale_color_viridis(discrete = F)+
  #scale_color_distiller(palette= 3, direction=1) +

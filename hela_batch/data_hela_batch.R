...
hela_batch_sample<-sc_group %>% arrange(Group)%>%filter(Group=="Hela")
data_hela_batch<-sc_data[,hela_batch_sample$sample]
pdf(file="./analysis/hela_a549_Hepg2/20231011_umap_hela_a549.pdf",height = 5,width = 6 )
overview_umap(data = data_hela_batch,data2 =hela_batch_sample,group = c("batch"),eclipse_level = 0.95,group_values = wheel("steelblue", num = 4))
dev.off()
...
pdf(file="./analysis/hela_a549_Hepg2/20231009_identi_hela_a549.pdf",height = 5,width = 6 )
ggplot(wide_df_hela_a549, aes(x=Group, y=mean,color=Group,fill=Group)) +
  ...
  scale_color_manual(values=wheel("steelblue", num = 4))+
  scale_fill_manual(values=wheel("steelblue", num = 4))+
...
pdf(file = "./analysis/hela_a549_Hepg2/20231011_corr_hela_a549.pdf",height = 5,width = 6)


corrplot(cor(log2(sc_data[,hela_a549_sample$sample]),method="pearson",use="complete.obs"),
         method="color",tl.col = "black",
         
         col=c(rep("white", 500),rep('grey', 100) ,COL2('RdYlBu', 200)),
         col.lim=c(0.5,1),
         #addCoef.col = "white",
         #type="upper")
)
...
Heatmap(
  ...
  show_column_dend = F,
  #row_km = 4,
  column_split = arrange(filter_data_hela_a549,Group)$Group,
  row_names_gp = gpar(fontsize = 10),
right_annotation = ha,
top_annotation = HeatmapAnnotation(df=filter_data_hela_a549 %>% arrange(Group)%>%
                                     dplyr::select(Group),
                                   col = list(Group=c("A549"=wheel("steelblue", num = 4)[1],
                                                      "Hela"=wheel("steelblue", num = 4)[2],
...
write.table(protein_data,file="20231011_sc_Data.txt",sep = "\t")
